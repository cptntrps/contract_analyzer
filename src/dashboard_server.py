...
class DashboardServer:
    ...
    def setup_routes(self):
        ...
        @self.app.route('/api/delete-contract/<contract_id>', methods=['DELETE'])
        def delete_contract(contract_id):
            ...
            try:
                ...
                for contract in self.contracts:
                    if contract.get('id') == contract_id or contract.get('name') == contract_id:
                ...
                file_path = Path(contract_to_delete['path'])
                if file_path.exists():
                    file_path.unlink()
                    logger.info(f"Deleted contract file: {file_path}")
                
                # Remove from contracts list
                self.contracts = [c for c in self.contracts if c.get('id') != contract_id and c.get('name') != contract_id]
                
                # Also remove any related files (redlined versions, etc.)
                base_name = contract_to_delete['name'].replace('.docx', '')
                for file in Path(self.upload_folder).glob(f"{base_name}*"):
                    if file.is_file():
                        file.unlink()
                        logger.info(f"Deleted related file: {file}")
        ...
        # Summary report endpoint removed
        ...
        # Summary report download endpoint removed
