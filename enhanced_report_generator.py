...
class EnhancedReportGenerator:
    ...
    def generate_all_reports(self, analysis_data: Dict[str, Any], 
        ...
        try:
            ...
            # 3. Generate Summary Report (.pdf and optionally .docx)
            if report_types.get('summary_pdf', True):
                summary_pdf_path = self.generate_summary_report_pdf(analysis_data, base_name)
                generated_files['summary_reports'].append(summary_pdf_path)
                
            if report_types.get('summary_docx', False):
                summary_docx_path = self.generate_summary_report_docx(analysis_data, base_name)
                generated_files['summary_reports'].append(summary_docx_path)
    ...
    def generate_summary_report_pdf(self, analysis_data: Dict[str, Any], base_name: str) -> str:
        """Generate Summary Report (.pdf) with improved readability and clear sections"""
        
        file_path = self.reports_dir / f"{base_name}_summary_report.pdf"
        ...
        # Title
        story.append(Paragraph("Contract Analysis Summary Report", title_style))
        ...
        exec_summary = f"""
        <b>Contract:</b> {analysis_data['contract']}<br/>
        <b>Template:</b> {analysis_data['template']}<br/>
        <b>Analysis Date:</b> {analysis_data['date']}<br/>
        <b>Total Changes:</b> {analysis_data['changes']}<br/>
        <b>Document Similarity:</b> {analysis_data['similarity']}%<br/>
        <b>Review Status:</b> {analysis_data['status']}<br/>
        <br/>
        <b>Change Breakdown:</b><br/>
        • Critical Changes: {total_critical}<br/>
        • Significant Changes: {total_significant}<br/>
        • Inconsequential Changes: {total_inconsequential}
        """
        ...
        <br/><br/>
        {self._get_risk_explanation(risk_level, analysis_data)}
        """
        
        story.append(Paragraph(risk_text, normal_style))
        story.append(Spacer(1, 0.3*inch))
        
        # Changes by Criticality (organized from highest to lowest priority)
        story.append(Paragraph("Changes by Criticality", heading_style))
        
        # Process in order of criticality: Critical -> Significant -> Inconsequential
        criticality_order = [
            ('critical', 'CRITICAL', '#dc3545'),
            ('significant', 'SIGNIFICANT', '#ffc107'),
            ('inconsequential', 'INCONSEQUENTIAL', '#28a745')
        ]
        ...
        for criticality_key, criticality_label, criticality_color in criticality_order:
            ...
            if changes:
                ...
                story.append(Paragraph(f'<font color="{criticality_color}"><b>{criticality_label} Changes ({len(changes)})</b></font>', subheading_style))
                
                # Add section description
                if criticality_key == 'critical':
                    description = "⚠️ These changes may significantly impact contract terms and require immediate legal review."
                elif criticality_key == 'significant':
                    description = "⚠️ These changes may affect contract performance and should be reviewed by stakeholders."
                else:
                    description = "✓ These changes are routine and typically do not require special attention."
                
                story.append(Paragraph(description, normal_style))
                story.append(Spacer(1, 0.1*inch))
                
                # List changes in narrative format (no tables)
                for i, change in enumerate(changes, 1):
                    # Change description
                    change_text = f"""
                    <b>{i}. {change.get('explanation', 'Change detected')}</b>
                    <br/>
                    <b>Template version:</b> {change.get('deleted_text', 'No text')[:200]}{'...' if len(change.get('deleted_text', '')) > 200 else ''}
                    <br/>
                    <b>Document version:</b> {change.get('inserted_text', 'No text')[:200]}{'...' if len(change.get('inserted_text', '')) > 200 else ''}
                    """
                    ...
                    change_style = ParagraphStyle(
                        ...
                        leftIndent=20,
                        spaceAfter=12,
                        borderLeft=f'3px solid {criticality_color}',
                        borderPadding=10
                    )
    ...
    def generate_summary_report_docx(self, analysis_data: Dict[str, Any], base_name: str) -> str:
        """Generate Summary Report (.docx) with improved readability and clear sections"""
        
        doc = Document()
        
        # Title
        title = doc.add_heading('Contract Analysis Summary Report', 0)
        title.alignment = WD_ALIGN_PARAGRAPH.CENTER
        ...
        exec_para.add_run("Contract: ").bold = True
        exec_para.add_run(f"{analysis_data['contract']}\n")
        exec_para.add_run("Template: ").bold = True
        exec_para.add_run(f"{analysis_data['template']}\n")
        exec_para.add_run("Analysis Date: ").bold = True
        exec_para.add_run(f"{analysis_data['date']}\n")
        exec_para.add_run("Total Changes: ").bold = True
        exec_para.add_run(f"{analysis_data['changes']}\n")
        exec_para.add_run("Document Similarity: ").bold = True
        exec_para.add_run(f"{analysis_data['similarity']}%\n")
        exec_para.add_run("Review Status: ").bold = True
        exec_para.add_run(f"{analysis_data['status']}\n")
        
        # Change breakdown
        doc.add_paragraph()
        breakdown_para = doc.add_paragraph()
        breakdown_para.add_run("Change Breakdown:\n").bold = True
        breakdown_para.add_run(f"• Critical Changes: {total_critical}\n")
        breakdown_para.add_run(f"• Significant Changes: {total_significant}\n")
        breakdown_para.add_run(f"• Inconsequential Changes: {total_inconsequential}")
        ...
        for criticality_key, criticality_label, criticality_color in criticality_order:
            ...
            if changes:
                ...
                if criticality_key == 'critical':
                    desc_para.add_run("⚠️ These changes may significantly impact contract terms and require immediate legal review.")
                elif criticality_key == 'significant':
                    desc_para.add_run("⚠️ These changes may affect contract performance and should be reviewed by stakeholders.")
                else:
                    desc_para.add_run("✓ These changes are routine and typically do not require special attention.")
                
                desc_para.runs[0].font.color.rgb = criticality_color
                ...
                for i, change in enumerate(changes, 1):
                    ...
                    change_para = doc.add_paragraph()
                    change_para.add_run(f"{i}. ").bold = True
                    change_para.add_run(change.get('explanation', 'Change detected')).bold = True
                    
                    # Template version
                    template_para = doc.add_paragraph()
                    template_para.paragraph_format.left_indent = Inches(0.5)
                    template_para.add_run("Template version: ").bold = True
                    template_text = change.get('deleted_text', 'No text')[:200]
                    if len(change.get('deleted_text', '')) > 200:
                        template_text += '...'
                    template_para.add_run(template_text)
                    
                    # Document version
                    document_para = doc.add_paragraph()
                    document_para.paragraph_format.left_indent = Inches(0.5)
                    document_para.add_run("Document version: ").bold = True
                    document_text = change.get('inserted_text', 'No text')[:200]
                    if len(change.get('inserted_text', '')) > 200:
                        document_text += '...'
                    document_para.add_run(document_text)
        ...
        if recommendations:
            ...
            no_rec_para.add_run("No specific recommendations at this time.")
        
        # Save document
        file_path = self.reports_dir / f"{base_name}_summary_report.docx"
        doc.save(file_path)
